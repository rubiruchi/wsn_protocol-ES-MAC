...
# step 3 cluster head selection according to LEACH protocol
...
# step 4 - Each cluster head broadcost advertisement packet
...
# Each member of node will recieve that advertisement packets
...
# step 5- Each member of the node will select that CH whose strength is strongest.
...
def step6(clusterList):
 ...
 # Each cluster head will broadcost a schedule packet in which it will allocate one CS and one DS to each member node
...
# The member of the node which has data will send the control packet in it's own CS
...
file_name='output_ldc_mac200.txt'
...
X1=list(map(int,listt_2[0].strip().split(' ')))
Y1=list(map(int,listt_2[1].strip().split(' ')))
firstDeadNodeRound = findFirstDeadNodeRound(X1,Y1)
lastDeadNodeRound = findLastDeadNodeRound(X1,Y1)
roundAxis=roundAxis[0:firstDeadNodeRound//2]+roundAxis[firstDeadNodeRound//2:6000:75]
numberOfDeadNodesAxis=numberOfDeadNodesAxis[0:firstDeadNodeRound//2]+numberOfDeadNodesAxis[firstDeadNodeRound//2:6000:75]
...
plt.text(lastDeadNodeRound-100, 205, lastDeadNodeRound)
plt.plot(roundAxis_new,numberOfDeadNodesAxis_smooth,'g',label='LDC-MAC')
