...
# Each cluster head will broadcost schedule packet in which it will allocate DS to each member
...
# Each member which has data will send data packet in it's own data slot.
def step8(clusterList):
	for i in range(len(clusterList)):
		...
		for j in range(len(clusterList[i])):
			if(clusterList[i][j]['isClusterHead']==True and clusterList[i][j]['isDead']==False):
				k = 100*8 #changed
				idleEnergy=recievingEnergy(k)/5
				clusterList[i][j]['energy']-= count_*recievingEnergy(k)
				clusterList[i][j]['energy']-= (total_nodes-count_)*idleEnergy
				if(clusterList[i][j]['energy']<=0):
					clusterList[i][j]['energy']=0
					clusterList[i][j]['isDead']=True
				break
...
# Each cluster head will aggregate the data and will send it to the base station.
...
file_name='output_es_mac200.txt'
...
roundAxis=roundAxis[0:firstDeadNodeRound//2]+roundAxis[firstDeadNodeRound//2:6000:90]
numberOfDeadNodesAxis=numberOfDeadNodesAxis[0:firstDeadNodeRound//2]+numberOfDeadNodesAxis[firstDeadNodeRound//2:6000:90]
